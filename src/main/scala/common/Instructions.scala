...
object Instructions {
  ...
  // CSR Access
  def CSRRW  = BitPat("b?????????????????001?????1110011")  // CSRの交換(read only)
  def CSRRS  = BitPat("b?????????????????010?????1110011")  // CSRの各ビットマスク付きセット(read only)
  def CSRRC  = BitPat("b?????????????????011?????1110011")  // CSRの各ビットマスク付きクリア(read only)
  def CSRRWI = BitPat("b?????????????????101?????1110011")  // CSRの即値との交換(read only)
  def CSRRSI = BitPat("b?????????????????110?????1110011")  // CSRの即値による各ビットマスク付きセット(read only)
  def CSRRCI = BitPat("b?????????????????111?????1110011")  // CSRの即値による各ビットマスク付きクリア(read only)
  // Change Level
