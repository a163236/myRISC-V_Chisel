...
#endif
#include <iostream>
class temp_api_t: public sim_api_t<VerilatorDataWrapper*> {
    public:
    temp_api_t(Vtemp* _dut) {
        dut = _dut;
        main_time = 0L;
        is_exit = false;
#if VM_TRACE
    ...
    void init_sim_data() {
        ...
        sim_data.inputs.push_back(new VerilatorIData(&(dut->io_di)));
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_addr)));
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_we)));
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_clk)));
        sim_data.outputs.push_back(new VerilatorIData(&(dut->io_dout)));
        sim_data.signals.push_back(new VerilatorCData(&(dut->reset)));
        sim_data.signal_map["temp.reset"] = 0;
    }
#if VM_TRACE
...
int main(int argc, char **argv, char **env) {
    ...
    Vtemp* top = new Vtemp;
    std::string vcdfile = "test_run_dir/temp_should/temp.vcd";
