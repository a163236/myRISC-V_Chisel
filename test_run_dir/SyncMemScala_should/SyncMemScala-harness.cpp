...
#endif
#include <iostream>
class SyncMemScala_api_t: public sim_api_t<VerilatorDataWrapper*> {
    public:
    SyncMemScala_api_t(VSyncMemScala* _dut) {
        dut = _dut;
        main_time = 0L;
        is_exit = false;
#if VM_TRACE
    ...
    void init_sim_data() {
        ...
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_datamport_req_typ)));
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_datamport_req_fcn)));
        sim_data.inputs.push_back(new VerilatorIData(&(dut->io_datamport_req_wdataD)));
        sim_data.inputs.push_back(new VerilatorIData(&(dut->io_datamport_req_addrD)));
        sim_data.inputs.push_back(new VerilatorIData(&(dut->io_instmport_req_raddrI)));
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_instmport_req_renI)));
        sim_data.outputs.push_back(new VerilatorIData(&(dut->io_datamport_resp_rdata)));
        sim_data.outputs.push_back(new VerilatorIData(&(dut->io_instmport_resp_rdata)));
        sim_data.signals.push_back(new VerilatorCData(&(dut->reset)));
        sim_data.signal_map["SyncMemScala.reset"] = 0;
    }
#if VM_TRACE
...
static SyncMemScala_api_t * _Top_api;
...
int main(int argc, char **argv, char **env) {
    ...
    VSyncMemScala* top = new VSyncMemScala;
    std::string vcdfile = "test_run_dir/SyncMemScala_should/SyncMemScala.vcd";
